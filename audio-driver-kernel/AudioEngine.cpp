...
bool AudioEngine::initHardware(IOService *provider)
{
    bool result = false;
    ...
    IOLog("AudioEngine[%p]::initHardware(%p)\n", this, provider);
...
IOAudioStream *AudioEngine::createNewAudioStream(IOAudioStreamDirection direction, void *sampleBuffer, UInt32 sampleBufferSize)
{
    IOAudioStream *audioStream;
...
void AudioEngine::stop(IOService *provider)
{
    IOLog("AudioEngine[%p]::stop(%p)\n", this, provider);
...
IOReturn AudioEngine::performAudioEngineStart()
{
    ...
    IOLog("AudioEngine[%p]::performAudioEngineStart()\n", this);
    fInterruptCount = 0;
    takeTimeStamp(false);
...
{
    IOLog("AudioEngine[%p]::performAudioEngineStop()\n", this);
    fAudioInterruptSource->cancelTimeout();
...
IOReturn AudioEngine::performFormatChange(IOAudioStream *audioStream, const IOAudioStreamFormat *newFormat, const IOAudioSampleRate *newSampleRate)
{
    IOLog("AudioEngine[%p]::peformFormatChange(%p, %p, %p)\n", this, audioStream, newFormat, newSampleRate);
...
IOReturn AudioEngine::convertInputSamples(const void *sampleBuf, void *destBuf, UInt32 firstSampleFrame, UInt32 numSampleFrames, const IOAudioStreamFormat *streamFormat, IOAudioStream *audioStream)
{
...
void AudioEngine::interruptOccured(OSObject* owner, IOTimerEventSource* sender)
{
    UInt64      thisTimeNS;
    uint64_t    time;
    SInt64      diff;
    
    AudioEngine* audioEngine = (AudioEngine*)owner;
...
void AudioEngine::handleAudioInterrupt()
{
